...
function BubblesSection() {
  ...
  const handleBubbleClick = async (bubble) => {
    ...
    setExplosions((prev) => [
      ...prev,
      { id: explosionId, left: bubble.left, top: bubble.top },
    ]);
  ...
  const handleMouseUp = useCallback(() => {
    setFloatingMessages((prev) =>
      prev.map((msg) =>
        msg.id === dragging ? { ...msg, isDragging: false } : msg
      )
    );
    setDragging(null);
  }, [dragging]);
          ...
          return (
            <div
              ...
              className={`floating-message ${
                isCurrentlyDragging ? "dragging" : "free-moving-text"
              }`}
              style={{
                left: `${message.left}%`,
                top: `${message.top}%`,
                ...
                animationDuration: isCurrentlyDragging
                  ? "none"
                  : animationDuration,
